...
class CocoDataset(Dataset):
    def __init__(
        ...
        if "train" in split:
            ...
            self.img_dir = os.path.join(self.img_dir, "train2014")
        elif "val" in split:
            # self.label_dir = os.path.join(self.label_dir, "val")
            self.img_dir = os.path.join(self.img_dir, "val2014")
...
class CocoCapClsDataset(CocoDataset):
    def __init__(self,
                 root_dir=None,
                 name_list_dir=None,
                 split='train',
        ...
        self.label_list = load_cls_label_list(name_list_dir=name_list_dir)
        if gt_annotation_file is not None:
            self.coco = COCO(gt_annotation_file)
        else:
            self.coco = None
    ...
    def __get_caption(self, img_name):
        if self.coco is not None:
            return self.coco.imgToAnns[int(img_name)][0]['caption']
        raise Exception("caption with captioning model is not yet ready!")
    ...
    def __getitem__(self, idx):
        ...
        caption = self.__get_caption(img_name)

        if self.aug:
            return img_name, image, cls_label, img_box, caption
        else:
            return img_name, image, cls_label, caption
