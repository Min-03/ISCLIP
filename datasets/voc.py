...
class VOC12CapClsDataset(VOC12Dataset):
    def __init__(self,
                 root_dir=None,
                 name_list_dir=None,
                 split='train',
        ...
        self.annotation_dir = os.path.join(root_dir, "Captions")
    ...
    def __get_caption(self, img_name):
        caption_file = os.path.join(self.annotation_dir, f"{img_name}.txt")
        if os.path.exists(caption_file):
            with open(caption_file, "r", encoding="utf-8") as f:
                return f.read()
        else:
            raise Exception(f"Cannot find a proper caption for {img_name}")
