...
class VOC12ClsDataset(VOC12Dataset):
    ...
    def __transforms_ver2(self, image):
        img_box = None
        if self.aug:
            image = np.array(image)
            # print('image', image.shape)
            ...
            if self.resize_range: 
                image = transforms.random_resize(
                    image, size_range=[self.crop_size, self.crop_size])
                pad = np.full(shape=(self.crop_size, self.crop_size, 3), fill_value=0, dtype=image.dtype)
                h, w, _ = image.shape
                pad[:h, :w] = image
                image = pad
    ...
    def __transforms_ver3(self, image):
        img_box = None
        if self.aug:
            image = np.array(image)
            # print('image', image.shape)
            resize_transform = Resize((self.crop_size, self.crop_size)) 
            image = resize_transform(torch.from_numpy(image).permute(2, 0, 1)).permute(1, 2, 0).numpy()
...
#         image = image.convert("RGB")
        ...
        if self.aug_ver == 1:
            ...
            image, img_box = self.__transforms_ver2(image=image)
        else:
            image, img_box = self.__transforms_ver3(image=image)
