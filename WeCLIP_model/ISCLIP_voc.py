...
from .fuse_transformer import TextFusionTransformer, TextFusionTransformer_ver2, TextFusionSubModule
import numpy as np
import clip
...
class ISCLIP(nn.Module):
    def __init__(self, num_classes=None, clip_model=None, embedding_dim=256, in_channels=512, dataset_root_path=None, device='cuda',
                 caption_file_dir=None, fuse_ver=1, n_layers=2):
        super().__init__()
        ...
        bg_text_features = zeroshot_classifier(BACKGROUND_CATEGORY, ['a clean origami {}.'], self.encoder)
        fg_text_features = zeroshot_classifier(new_class_names, ['a clean origami {}.'], self.encoder)
        
        self.register_buffer("bg_text_features", bg_text_features)
        self.register_buffer("fg_text_features", fg_text_features)
        ...
        fuse_module_dict = {1:TextFusionTransformer, 2:TextFusionTransformer_ver2, 3:TextFusionSubModule}
        self.fuse_module_fg = fuse_module_dict.get(fuse_ver)(embed_dim=512, heads=8, layers=n_layers).to(torch.float16)
        # self.fuse_module_bg = fuse_module_dict.get(fuse_ver)(embed_dim=512, heads=8, layers=n_layers).to(torch.float16)
        


    def get_param_groups(self):

        param_groups = [[], [], [], [], []]  # backbone; backbone_norm; cls_head; seg_head; fuse_module
        ...
        for param in list(self.fuse_module_fg.parameters()):
            param_groups[4].append(param)
        # for param in list(self.fuse_module_bg.parameters()):
        #     param_groups[4].append(param)

        return param_groups
    
    def refine_text(self, caption_feats, cls_label):
        fg_text_feat_list = []
        cap_num = 0
        for i in range(len(self.fg_text_features)):
            if cls_label[i] == 0:
                fg_text_feat_list.append(self.fg_text_features[i])
                continue
            ref_cap = caption_feats[cap_num].unsqueeze(0)
            cap_num += 1
            refined_feat = self.fuse_module_fg(self.fg_text_features[i].unsqueeze(0), ref_cap, ref_cap, class_idx=i)
    ...
    def forward(self, img, img_names='2007_000032', mode='train', cls_labels=None):
        ...
        for i, img_name in enumerate(img_names):
            ...
            if mode == "train":
                ...
                specific_caption_feats = specific_caption_feats.detach()
                fg_text_feats, bg_text_feats = self.refine_text(specific_caption_feats, cls_label)
            else:
