...
def draw_activation(img, attn, file_name, title):
    img_denorm = denormalize_img(img.unsqueeze(0)).squeeze(0)
    img_denorm = img_denorm.permute(1, 2, 0).cpu().detach().numpy()
    img_plt = plt.imshow(img_denorm)
    plt.imshow(attn.squeeze(0).cpu().detach().numpy(), cmap="jet", alpha=0.6, extent=img_plt.get_extent())
