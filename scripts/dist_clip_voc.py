...
def train(cfg):
    ...
    for n_iter in range(cfg.train.max_iters):
        ...
        if args.refine_cam:
            segs, cam, attn_pred = WeCLIP_model.forward_with_fuse(inputs.cuda(), img_name, cls_labels=cls_labels)
        else:
            segs, cam, attn_pred = WeCLIP_model(inputs.cuda(), img_name, mode=args.train_mode, cls_labels=cls_labels)
